...
#### 基本モード切り替え
- **STOPモード**: `PSボタン`（button[16]） - 🔴 赤LED
- **JOYモード**: `右小ボタン(Options)`（button[6]） - 🟢 緑LED  
- **DPADモード**: `左小ボタン(Share)`（button[7]） - 🔵 青LED

#### ライントレースモード
- **切り替え**: `左右スティック同時押し込み`（button[10] + button[9]） - ⚪ 白LED
