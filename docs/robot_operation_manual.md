...
#### 基本モード切り替え
- **STOPモード**: `Share`ボタン（button[15]） - 🔴 赤LED
- **JOYモード**: `Options`ボタン（button[6]） - 🟢 緑LED  
- **DPADモード**: `L1`ボタン（button[4]） - 🔵 青LED

#### ライントレースモード
- **切り替え**: `左右スティック同時押し込み`（button[7] + button[8]） - ⚪ 白LED
...
特殊機能:
- L2トリガー（axes[4]）: 左回転（手動）
- R2トリガー（axes[5]）: 右回転（手動）
- R1ボタン（button[5]）: 前後反転トグル
```
...
#### 昇降制御（INIT・STOPPEDモードで有効）
```
上昇: △ボタン（button[3]）- 押している間上昇
下降: ×ボタン（button[0]）- 押している間下降
停止: ボタンを離す
```
...
#### 手動射出（非推奨・デバッグ用）
```
手動押し出し: □ボタン（button[2]）- 押している間のみ動作
```
...
| ボタン/軸 | 番号 | 足回り機能 | 上物機能 | 備考 |
|-----------|------|------------|----------|------|
| ×ボタン | button[0] | - | 昇降下降 | INIT/STOPPEDで有効 |
| ○ボタン | button[1] | - | **自動射出** | STOPPEDで有効 |
| □ボタン | button[2] | - | 手動押し出し | デバッグ用 |
| △ボタン | button[3] | - | 昇降上昇 | INIT/STOPPEDで有効 |
| L1ボタン | button[4] | **DPADモード** | - | 🔵 青LED |
| R1ボタン | button[5] | 前後反転 | - | トグル切り替え |
| Optionsボタン | button[6] | **JOYモード** | - | 🟢 緑LED |
| Shareボタン | button[7] | ライントレース※ | - | ⚪ 白LED |
| - | button[8] | ライントレース※ | - | ⚪ 白LED |
| 右スティック押込 | button[9] | ライントレース※ | - | ⚪ 白LED |
| 左スティック押込 | button[10] | ライントレース※ | - | ⚪ 白LED |
| 方向パッド上 | button[11] | DPAD前進 | - | DPADモード時 |
| 方向パッド下 | button[12] | DPAD後退 | - | DPADモード時 |
| 方向パッド左 | button[13] | DPAD左移動 | - | DPADモード時 |
| 方向パッド右 | button[14] | DPAD右移動 | - | DPADモード時 |
| Shareボタン | button[15] | **STOPモード** | - | 🔴 赤LED |
| 左スティック左右 | axes[0] | 左右移動 | - | JOYモード時 |
| 左スティック上下 | axes[1] | 前後移動 | - | JOYモード時 |
| 右スティック左右 | axes[2] | 回転 | - | JOYモード時 |
| L2トリガー | axes[4] | 左回転 | システム準備※ | 手動回転 |
| R2トリガー | axes[5] | 右回転 | システム準備※ | 手動回転 |
...
**📞 サポート情報**
- 開発チーム: Rox Robot Project
- ドキュメント更新日: 2025/08/14
- バージョン: v2.0
- 対応ROS2: Jazzy
