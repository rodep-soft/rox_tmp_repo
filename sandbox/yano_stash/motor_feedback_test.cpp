...
class MotorController {
 public:
  MotorController(const std::string& port_name)
      : port_(io_context_, port_name) {
    init();
  }
  ...
  void motor_control(bool brake = false) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(3));  // Wait for the command to be processed
    ...
    if (auto response = receive_feedback()) {
      ...
      uint16_t position =
          static_cast<uint16_t>((feedback[6] << 8) | feedback[7]);
  ...
  std::optional<std::vector<uint8_t>> receive_feedback() {
    ...
    try {
      uint8_t crc = calc_crc8_maxim(
          std::vector<uint8_t>(feedback.begin(), feedback.begin() + 9));
      if (crc != feedback[9]) {
