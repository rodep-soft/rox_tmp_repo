...
// using namespace std::chrono_literals; // using namespaceは関数内または特定のスコープでの使用を推奨
...
public:
    ...
    bool initialize() {
        ...
        if (!trig_ || !echo_) {
            std::cerr << "Error: Failed to get GPIO lines. TRIG_LINE: " << TRIG_LINE << ", ECHO_LINE: " << ECHO_LINE << std::endl;
        ...
        if (gpiod_line_request_output(trig_, "hcsr04_trig", 0) < 0) {
            std::cerr << "Error: Failed to request trigger line (GPIO" << TRIG_LINE << ") as output." << std::endl;
            cleanup();
            return false;
        }
        
        if (gpiod_line_request_input(echo_, "hcsr04_echo") < 0) {
            std::cerr << "Error: Failed to request echo line (GPIO" << ECHO_LINE << ") as input." << std::endl;
            cleanup();
            return false;
        }
    ...
    double measureDistance() {
        ...
        // 距離を計算 (メートル単位)
        auto duration_us = std::chrono::duration_cast<std::chrono::microseconds>(echo_end_time - echo_start_time).count();
...
public:
    ...
    UltrasonicSensorNode() : Node("ultrasonic_sensor_node") {
        ...
        if (!gpio_sensor_.initialize()) {
            RCLCPP_FATAL(this->get_logger(), "Failed to initialize GPIO for ultrasonic sensor. Shutting down ROS node.");
