...
MotorController::MotorController(rclcpp::Logger logger) : serial_port_(io_context_), logger_(logger), 
                                           buffer_(), reading_(false) {}
...
void MotorController::send_velocity_commands_sequential(const std::vector<std::pair<uint8_t, int16_t>>& commands, bool brake) {
...
bool MotorController::validate_motor_response(const std::vector<uint8_t>& response, uint8_t expected_id) {
    if (response.size() < 10) {
      return false;
    }
