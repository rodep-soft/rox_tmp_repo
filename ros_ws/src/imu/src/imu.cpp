 ...
 * @file BNO055.cpp
 *
 *  @mainpage BNO055 Orientation Sensor
 *
 *  @section intro_sec Introduction
 *
 *  This is a C++ library for the BNO055 orientation sensor using standard C++
 *
 ...
 */
bool BNO055::begin(opmode_t mode) {
  // Open I2C device
  _i2c_fd = open(_i2c_device.c_str(), O_RDWR);
  if (_i2c_fd < 0) {
    std::cerr << "Failed to open I2C device: " << _i2c_device << std::endl;
    return false;
  }
 ...
 *            OPERATION_MODE_NDOF]
 */
void BNO055::setMode(opmode_t mode) {
  _mode = mode;
 ...
 *           REMAP_CONFIG_P7]
 */
void BNO055::setAxisRemap(axis_remap_config_t remapcode) {
 ...
 *           REMAP_SIGN_P7]
 */
void BNO055::setAxisSign(axis_remap_sign_t remapsign) {
  opmode_t modeback = _mode;
 ...
 *          revision info
 */
void BNO055::getRevInfo(rev_info_t *info) {
  uint8_t a, b;
 ...
 */
Vector<3> BNO055::getVector(vector_type_t vector_type) {
  Vector<3> xyz;
  uint8_t buffer[6];
  memset(buffer, 0, 6);
...
bool BNO055::write8(bno055_reg_t reg, uint8_t value) {
  ...
  ssize_t result = write(_i2c_fd, buffer, 2);
  return result == 2;
}
...
bool BNO055::readLen(bno055_reg_t reg, uint8_t *buffer, uint8_t len) {
  ...
  if (write(_i2c_fd, reg_buf, 1) != 1) {
    return false;
  }
  return read(_i2c_fd, buffer, len) == len;
}
